...
nx=2*(xmax-xmin); %number of cells
cfl_target=0.8; %target courant Courant number
...
b=0.5*exp(-0.005*(xcenters-35).^2)+2.0*exp(-0.005*(xcenters-75).^2);
%b=0*xcenters;
%b=0.5*(xcenters>30).*(xcenters<40);

%initial conditions
%h_0=max(0,1-b)+0.2*exp(-(0.1*xcenters).^2);
%h_0=max(0,-0.05+0.2*exp(-(0.1*xcenters).^2))
%h_0=max(0,1-b).*(xcenters>-60).*(xcenters<-30);
h_0=(max((0+0.6*(xcenters<-30))-b,0));
hu_0=0*0.1*h_0;
...
for n=1:nmax
    ...
    for i=3:nx+3
        ...
        fnorm1=0; fnorm2=0; fnorm3=0;
        dotl1=dotr1; dotl2=dotr2; dotl3=dotr3;
        dotr1=0; dotr2=0; dotr3=0;
        for m=1:numel(fwave(i,1,:));
            fnorm1=fnorm1+fwave(i,1,m)^2;
            fnorm2=fnorm2+fwave(i,2,m)^2;
            fnorm3=fnorm3+fwave(i,3,m)^2;
            dotr1=dotr1+fwave(i,1,m)*fwave(i+1,1,m);
            dotr2=dotr2+fwave(i,2,m)*fwave(i+1,2,m);
            dotr3=dotr3+fwave(i,3,m)*fwave(i+1,3,m);
        end
...
%         I2=i-sign(s(i,2));
%         I3=i-sign(s(i,3));
%         theta1=(dot(squeeze(fwave(I1,1,:)),Z1))/dot(Z1,Z1);
%         theta2=(dot(squeeze(fwave(I2,2,:)),Z2))/dot(Z2,Z2);
%         theta3=(dot(squeeze(fwave(I3,3,:)),Z3))/dot(Z3,Z3);
    ...
    for i=3:numel(h_n-2)
        if h_n(i)<drytol
            if h_n(i)<0
                strcat('WARNING: NEGATIVE DEPTH: h=',num2str(h_n(i)))
            end
            h_n(i)=0;
            hu_n(i)=0;
        end
